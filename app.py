...
async def refresh_cache():
    """Force-rebuild the prop cache. Useful when prices look stale on the phone."""
    await cache.build(kalshi)
    log.info("Cache force-refreshed via API")
    return {"hr": len(cache.hr_cache), "hits": len(cache.hits_cache)}
...
async def place_trade(req: TradeRequest):
    ...
    # Game profile: IOC so we don't leave a resting order if the price has moved.
    order_type = "ioc" if profile == "game" else "limit"
    await kalshi.place_order(prop.ticker, "yes", yes_cents, count, order_type)

    # Game profile: sell the position at sell_cents once the market reprices.
    # Home profile: hold to $1.00 settlement — no action needed.
    if profile == "game":
        _ticker  = prop.ticker
        _sell_at = sell_at
        _count   = count
        ...
        async def flip():
            while time.monotonic() < _deadline:
                try:
                    await kalshi.place_order(_ticker, "yes", _sell_at, _count, "limit", action="sell")
                    log.info(f"SOLD {_ticker} @ {_sell_at}¢")
                    return
                except Exception as e:
                    log.warning(f"Sell retry: {e}")
                    await asyncio.sleep(3)
            log.warning(f"Flip window expired for {_ticker}")
